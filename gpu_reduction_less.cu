...
__global__ void gpuReductionOptimized(int* input, int* output, int size) {
...
int main() {
    ...
    for (int idx = 0; idx < 5; idx++) {
        ...
        double t0 = get_clock();
        gpuReductionOptimized<<<blocks, threads>>>(d_input, d_output, size);
        cudaDeviceSynchronize();
        double t1 = get_clock();

        cudaMemcpy(output, d_output, sizeof(int), cudaMemcpyDeviceToHost);

        printf("GPU Reduction Less Thread Divergent - Size %d: %f s\n", size, t1 - t0);
