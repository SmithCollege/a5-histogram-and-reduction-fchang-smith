...
__global__ void gpuHistogramStrided(int* data, int* histogram, int size, int bins, int range) {
...
int main() {
    ...
    for (int idx = 0; idx < 5; idx++) {
        ...
        double t0 = get_clock();
        gpuHistogramStrided<<<blocks, threads>>>(d_data, d_histogram, size, bins, range);
        cudaDeviceSynchronize();
        double t1 = get_clock();

        cudaMemcpy(histogram, d_histogram, bins * sizeof(int), cudaMemcpyDeviceToHost);

        printf("GPU Histogram (Strided) - Size %d: %f s\n", size, t1 - t0);
